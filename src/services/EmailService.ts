...
// Extract ALL env vars before any usage — never reference import.meta.env inside strings
...
const SENDER_NAME  = (import.meta.env.VITE_BREVO_SENDER_NAME as string) || 'RentCheck';
const APP_URL      = (import.meta.env.VITE_APP_URL           as string) || 'https://your-project-id.web.app';

// In dev, Vite proxies /brevo → https://api.brevo.com to avoid CORS.
// In production (Firebase Hosting), the direct URL works fine.
const BREVO_API_URL = import.meta.env.DEV
  ? '/brevo/v3/smtp/email'
...
export async function sendVerificationEmail({ toEmail, toName }: SendEmailParams): Promise<void> {
  // Build the HTML string using plain variables — no import.meta.env inside here
  const htmlContent = `
    <!DOCTYPE html>
    <html>
